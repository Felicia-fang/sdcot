...
def parse_prediction_dynamic_to_pseudo_bboxes(end_points, config_dict, point_clouds,threshold_class={0:0.95,1:0.96,2:0.9,3:0.91,4:0.91,5:0.92,6:0.9,7:0.9,8:0.9,9:0.92,10:0.9,11:0.91,12:0.92,13:0.9}):
    """ Parse predictions to OBB parameters and suppress overlapping boxes
    ...
    for i in range(K):
        ...
        if choose==0:
            threshold_new=threshold_class[pred_sem_cls_max_indices[i]]
        elif choose == 1:
        ...
        if pred_mask[i] == 1 and obj_prob[i] > threshold_new and \
           pred_sem_cls_probs[i] > config_dict['cls_conf_thresh']:
            bbox_param = np.zeros((11))
            bbox_param[0:3] = pred_center[i]
            bbox_param[3:6] = pred_size[i]
            bbox_param[6] = pred_heading_angle[i]
            bbox_param[7] = pred_sem_cls[i]
            # add the probability of the class to the last column
            bbox_param[8] = obj_prob[i]
            bbox_param[9]=obj_logits[i][0]
            bbox_param[10]=obj_logits[i][1]
            pred_bboxes.append(bbox_param)
            conf_scores.append(obj_prob[i]*pred_sem_cls_probs[i])
...
def parse_prediction_to_pseudo_bboxes(end_points, config_dict, point_clouds):
    ...
    pred_bboxes = []  # a list (len: num of predictions) of pred_box_params
    conf_scores = []
    threshold_class={0:0.95,1:0.96,2:0.9,3:0.91,4:0.91,5:0.92,6:0.9,7:0.9,8:0.9,9:0.92,10:0.9,11:0.91,12:0.92,13:0.9}
    for i in range(K):
        # print("thresh",config_dict['obj_conf_thresh']*normalized_counts[pred_sem_cls_max_indices[i]])
        if choose==0:
            threshold_new=threshold_class[pred_sem_cls_max_indices[i]]
